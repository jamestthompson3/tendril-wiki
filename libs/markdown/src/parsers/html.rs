                ...
                "[" => {
                    match parser.current_state() {
                        ...
                        ParserState::LinkStart => {
                            // println!("  \x1b[48;5;57mStarting to parse\x1b[0m");
                ...
                "]" => {
                    match parser.current_state() {
                        ParserState::LinkEnd => {
                            ...
                            // println!("Formatting Link: {}", link_text);
                            wiki_link_location.clear();
                            parser.send(ParserState::Accept);
                            Event::Html(
                                format!(r#"<a href="{}">{}</a>"#, link_location, text).into(),
                            )
                        }
                        ParserState::LocationParsing => {
                            // println!("  \x1b[48;5;132mStopping parse\x1b[0m");
